...
const MyTodo = () => {
    ...
    useEffect(() => {
        const myToDo = localStorage.getItem("todoAdd")
        setMyToDo(JSON.parse(myToDo))
    }, [])

    // const handleDelete = (title) => {
    //     const newMyToDo = myToDo.filter(todo => todo.title != title)
    //     localStorage.setItem("todoAdd", JSON.stringify(newMyToDo))
    //     setMyToDo(newMyToDo)
    // }
          ...
          `,
            ...
            if (result.isConfirmed) {
                ...
                const updatedMyToDo = myToDo.map((todo) =>
                    todo.title === title ? { ...todo, title: newTitle, description: newDescription } : todo
                );
                localStorage.setItem("todoAdd", JSON.stringify(updatedMyToDo));
